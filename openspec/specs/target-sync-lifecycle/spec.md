...
The system SHALL delete cloned events from the target calendar when their source events are removed during sync.

#### Scenario: Source event deleted via sync delta
- **WHEN** a sync delta includes a deleted event that has a TargetEventMapping
- **THEN** the system calls `deleteEvent()` on the target provider for each mapping
- **AND** the TargetEventMapping record is deleted
...
The system SHALL detect and clean up TargetEventMappings whose source events no longer exist.

#### Scenario: Source event deleted outside sync flow
- **WHEN** `cloneToTarget()` runs and finds mappings where the source event no longer exists in the database
- **THEN** the system calls `deleteEvent()` on the target provider for each orphaned mapping
...
The system SHALL only update target events when the source event has changed since the last sync to the target.
...
- **THEN** the system calls `updateEvent()` on the target provider
