    ...
    <AppShell section="bookings">
      <div className="mx-auto max-w-3xl space-y-6">
        <div className="flex items-center justify-between">
          <h1 className="font-display text-2xl font-bold tracking-tight">Buchungen</h1>
        ...
        <Card>
          <CardHeader><h2 className="text-lg font-semibold">Anstehend</h2></CardHeader>
          <CardBody>
            {upcoming.length === 0 ? (
              <p className="text-default-400">Keine anstehenden Buchungen.</p>
            ) : (
        ...
        {past.length > 0 && (
          <Card>
            <CardHeader><h2 className="text-lg font-semibold">Vergangen / Storniert</h2></CardHeader>
            <CardBody>
              <div className="space-y-3">
                {past.map((b) => (
        ...
        <Modal isOpen={!!cancelId} onClose={() => setCancelId(null)}>
          <ModalContent>
            ...
            <ModalBody>
              <p>Bist du sicher, dass du diese Buchung stornieren möchtest? Der Kalendereintrag wird gelöscht.</p>
            </ModalBody>
            <ModalFooter>
              <Button variant="light" onPress={() => setCancelId(null)}>Behalten</Button>
              <Button color="danger" isLoading={cancelling} onPress={handleCancel}>Buchung stornieren</Button>
            </ModalFooter>
          </ModalContent>
        </Modal>
      </div>
